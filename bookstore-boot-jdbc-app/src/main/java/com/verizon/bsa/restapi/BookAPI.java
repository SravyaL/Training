...
public class BookAPI {
	...
	@GetMapping("/{eid}")
	public ResponseEntity<Book> getBookAction(@PathVariable("eid") int eid) {
		ResponseEntity<Book> resp = null;
		Book b = bookService.getBookById(eid);
		if (b == null)
	...
	public ResponseEntity<Void> deleteBookAction(@PathVariable("eid") int eid) {
		...
		boolean isDeleted = bookService.deleteBookById(eid);
	...
	public ResponseEntity<Book> updateBookAction(@RequestBody Book book) {
		...
		if (bookService.getBookById(book.getEid()) == null) {
			resp = new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}else {
