			 ...
			 {//				results = Collections.singletonList(cByMobNum);	
				 ...
				 case "emailId" :			
					 ...
					 if(cByEmail!=null)			{				
						 ...
						 case "lastName" :			
							 results=service.findByLnm(searchValue);			
							 if(results != null && results.size() != 0)			{				
								 resp=new ResponseEntity<>(results,HttpStatus.OK);			}			
							 else			{				
								 resp= new ResponseEntity<>(HttpStatus.NOT_FOUND);			}			break;		default :			resp = new ResponseEntity<>(HttpStatus.BAD_REQUEST);			break;		}
			

	

	if(results==null)resp=new ResponseEntity<List<Contact>>(HttpStatus.BAD_REQUEST);else if(results.size()==0)resp=new ResponseEntity<List<Contact>>(HttpStatus.NOT_FOUND);else resp=new ResponseEntity<List<Contact>>(results,HttpStatus.OK);return resp;}
